...
rm -rf APP/data/freebayesfile/vcffile/*.vcf && rm -rf APP/data/freebayesfile/metrics/*.txt
Nbargument=$#
...
do
    ...
    freebayes --ploidy $Ploidy -f $Pathogene APP/data/Bam/Mapped/BamFreebayes/"${id}".dup.bam > APP/data/freebayesfile/vcffile/"${id}"_variants.vcf 

done
mv -f *metrics.* APP/data/freebayesfile/metrics/
rm -rf 100